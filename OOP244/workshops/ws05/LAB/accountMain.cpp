...
using namespace sdds;
    ...
    void displayAccounts(const Account* acc, int num) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            cout << (i + 1) << "|";    // ROW |001|
            cout.fill(' ');
            acc[i].display() << "|" << endl;    // ACC# | 11111 | (account number)
        }
        cout << "+---+-------+--------------+" << endl;
    }

    // displayAccounts(mixed, 10); 왜 parameter 2개? mixed array 10개 출력하라
    void displayAccount(const Account& acc) {
        displayAccounts(&acc, 1); // 이건 또 왜 하나
    }
    ...
    int main() {
        ...
        Account mixed[10] = {
           {11111, 111.11},
           {22222, 222.22},
           {333, 333.333},   // invalid: BAD | ACCOUNT
           {44444, 4444.44},
           {55555, 555.55},
           {66666, -66.66},  // invalid: : BAD | ACCOUNT
           {77777, 777.77}
        };
